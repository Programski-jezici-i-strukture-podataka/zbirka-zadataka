#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#define MAX_PASTA 20
#define MAX_NAZIV 31
#define MAX_DECIJA 3
#define MAX_SREDJEN_NAZIV 39

typedef struct pasta_st 
{
    char naziv[MAX_NAZIV];
    double cena;
    int velicina;
    char decija[MAX_DECIJA];
} PASTA;

FILE *safe_fopen(char *, char *, int);
void ucitaj_paste(FILE *, PASTA *, int *);
double suma_cena(PASTA *, int);
double prosek_cena(PASTA *, int);
void ispisi_vece(PASTA *, int, int);
void sredjeni_nazivi(FILE *, PASTA *, int);

int main(int argc, char **argv) 
{

    PASTA paste[MAX_PASTA];
    int n, trazena_velicina;
    double prosek;
    FILE *ulazna, *izlazna;

    if(argc != 2) 
    {
        printf("Primer poziva: %s 100\n", argv[0]);
        exit(1);
    }

    ulazna = safe_fopen("paste.txt", "r", 3);
    ucitaj_paste(ulazna, paste, &n);
    fclose(ulazna);
    
    prosek = prosek_cena(paste, n);
    printf("Prosecna pasta za zube kosta: %.2lf dinara.\n", prosek);
    
    trazena_velicina = atoi(argv[1]);
    ispisi_vece(paste, n, trazena_velicina);
    
    izlazna = safe_fopen("sredjeni_nazivi.txt", "w", 4);
    sredjeni_nazivi(izlazna, paste, n);
    fclose(izlazna);
    
    return 0;
}

FILE *safe_fopen(char *ime, char *rezim, int kod_greske)
{
    FILE *pf = fopen(ime, rezim);
    
    if (pf == NULL)
    {
        printf("Greska pri otvaranju %s!\n", ime);
        exit(kod_greske);
    }
    
    return pf;
}

void ucitaj_paste(FILE *pf, PASTA *p, int *pn) 
{

    int i = 0;

    while(fscanf(pf, "%s %lf %d %s", p[i].naziv, &p[i].cena, &p[i].velicina, p[i].decija) != EOF)
    {
        i++;
    }
    
    *pn = i;
}

double suma_cena(PASTA *p, int n) {
    int i;
    double suma = 0;
    
    for(i = 0; i < n; i++) 
    {
        suma += p[i].cena;
    }
    
    return suma;
}

double prosek_cena(PASTA *p, int n) {
    return suma_cena(p, n) / n;
}

void ispisi_vece(PASTA *p, int n, int velicina) {
    int i, nadjena = 0;
    PASTA *najjeftinija = NULL;
    
    printf("Paste za zube od bar %dml: ", velicina);
    for(i = 0; i < n; i++) 
    {
        if(p[i].velicina > velicina) 
        {
            printf("%s ", p[i].naziv);
            nadjena = 1;
            if (najjeftinija == NULL || najjeftinija->cena > p[i].cena)
            {
                najjeftinija = &p[i];
            }
        }
    }
    printf("\n");
    if (nadjena)
    {
        printf("Najjefinija od njih je: %s!\n", najjeftinija->naziv);
    }
    else
    {
        printf("Nije nadjena pasta za zube veca od unete velicine!\n");
        exit(2);
    }
}

void sredjeni_nazivi(FILE *pf, PASTA *p, int n) {
    int i, j;
    for(i = 0; i < n; i++) 
    {
        char naziv[MAX_SREDJEN_NAZIV];
        strcpy(naziv, p[i].naziv);
        for(j = 0; j < strlen(naziv); j++) 
        {
            if (naziv[j] >= 'a' && naziv[j] <= 'z') 
            {
                naziv[j] -= 32;
            }
            if (naziv[j] == '_') 
            {
                naziv[j] = ' ';
            }
        }
        if (strcmp(p[i].decija, "da") == 0) 
        {
            strcat(naziv, " za decu");
        }
        fputs(naziv, pf);
        fputs("\n", pf);
    }
}
